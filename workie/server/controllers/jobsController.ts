...
const getJobs = async (req: any, res: any) => {
  ...
  const { userId }: any = jwt.verify(user, JWT_SECRET);
  try {
    const getJobs = await JobsModel.find({ createdBy: userId });
...
const getJob = async (req: any, res: any) => {
  ...
  // }
  try {
    const getJobDetails = await JobsModel.findById(jobType);
    if (!getJobDetails) {
...
const addRegisteredUser = async (req: any, res: any) => {
  ...
  console.log(userId);
  try {
    const job: any = await JobsModel.findById(jobId);
...
const deleteRegisteredUser = async (req: any, res: any) => {
  ...
  try {
    const deleteUser = await JobsModel.findById(jobId);
