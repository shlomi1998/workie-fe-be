...
const SignedUpForWork = () => {
  ...
  const handleApproval = (jobId: any, userId: any) => {
    ...
    setUserId(userId);
    if (jobId && userId) {
      console.log(`/api/v1/jobs/${jobId}/${userId}`);
      const fetchJob = async () => {
        try {
          const response = await axios.delete(
            `/api/v1/jobs/${jobId}/${userId}`
          );
  ...
  return (
    <div className="wrapsRegisteredUsers">
      <div className="navBar">
        <span className="navBar__title">registered for work</span>
      </div>
      <div>{arrayDiv}</div>
...
export default SignedUpForWork;
