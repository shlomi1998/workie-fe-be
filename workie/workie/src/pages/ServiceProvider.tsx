...
import { TopNavbarService, BottomNavBarMainScreen } from "../components/index";
...
const ServiceProvider = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const ArrayDiv: any = [];
        const response = await axios.get("/api/v1/services/getJobs");
          ...
          ArrayDiv.push(
            <div className="addingWork__myWork__div" key={i}>
              ...
              <span
                ...
                className="fa-solid fa-trash addingWork__myWork__div__iconTrash trashIcon "
              ></span>
  ...
  const navigateToEmployerProfile = (jobId: any) => {
    ...
    navigate("/SignedUpForServices");
  };

  return (
    <div className="wrapsServiceProvider">
      <TopNavbarService />
      <div className="addingWork">
        ...
        <div
          className={
            isTrue ? "addingWork__deleteJob" : "addingWork__deleteJobHide"
          }
        >
          ...
          <button
            onClick={handleClick}
            className="addingWork__deleteJob__buttonCancelation"
          >
            Cancelation
          </button>
          <button
            onClick={handleApproval}
            className="addingWork__deleteJob__buttonApproval"
          >
            Approval
          </button>
        </div>
